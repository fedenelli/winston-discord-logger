...
module.exports = class DiscordLogger extends Transport {
	constructor(options) {
		...
		this.tagEveryoneOnError = options.tagEveryoneOnError || false;
		...
		webhooks = webhooks.map(webhook => {
			if (typeof webhook === 'string') {
				const webhookURLRegex = /(https?):\/\/([a-z0-9]*).*(discordapp.com|discord.com)\/api(?:\/v([0-9]+))*\/webhooks\/([0-9]{1,20})\/([a-z0-9_-]+)/gi;
	...
	log(info, callback) {
		...
		return new Promise((resolve, reject) => {
			...
			const fields = Object.keys(meta).map(key => {
				...
				return { name: key, value: typeof meta[key] !== 'string' ? JSON.stringify(meta[key]) : meta[key], inline: inline }
			});
			...
			let body = JSON.stringify({
				embeds: [{
					...
					fields: [...fields, ...this.metadata],
					timestamp: new Date().toISOString()
				}],				
				content: this.tagEveryoneOnError && level === 'error' ? '\@everyone' : null,
			});
			// Post details
			const post = {
